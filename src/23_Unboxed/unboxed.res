/* Unboxed
 * Rescript에서 싱글 페이로드를 갖는 베리언트와 싱글 필드를 갖는 레코드를 살펴보자.
 */
 ...
 // 성능이 중요하거나 특정 자바스크립트 인터롭 상황의 경우 Rescript가 제공하는 unwrap(또는 unbox) 기능을 사용할 수 있다.
 // Unbox는 단일 생성자와 싱글 페이로드로 구성된 배리언트와 싱글 필드의 레코드가 JS로 변환될 때, 해당 객체의 래퍼를 벗겨낸다.
 // 즉 Rescript의 unbox의 기능은 불필요한 객체 대신 변수로 변환한다.
