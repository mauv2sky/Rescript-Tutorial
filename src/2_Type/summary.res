...
/* 추론
 * 값을 통해 추론합니다. 아래의 add 함수가 두 개의 int를 덧셈 연산하는 것을 통해
 * 반환값이 int임을 추론할 수 있다.
 */
let score = 10
let add = (a, b) => a + b
...
/* 타입 매개 변수
 * 타입은 다른 언어의 제네릭과 유사한 매개 변수를 허용합니다.
 * 중복을 없애기 위해 고안되었으며, 매개변수 이름 앞에 ' 를 기입하면 된다.
 * 타입 시스템은 (int, int, int)와 같은 타입 선언이 필요 없습니다.
 * 타입 매개 변수를 허용하지 않은 경우, arrayOfString, arrayOfInt 등의 표준 라이브러리의 타입을 정의해야 할 것이다.
 */
 // 일반 표기 방식
