 ...
 * 1. 강함 
 *      보통의 자바스크립트에서는 코드가 실행될 때(런타임 시) 변수 타입이 변경될 수 있다.
 *      이런 상황은 읽기 또는 디버깅 시에 코드를 이해하는 것을 어렵게 만든다.
 *      Rescript의 타입은 다른 타입으로 변하지 않기 때문에 위와 같은 혼란을 야기시키지 않는다.
 ...
 * 값의 타입을 직접 명시하는 타입 주석을 선택적으로 작성할 수 있다.
 * 만약 타입 주석이 Rescript가 추론한 타입과 일치하지 않으면, Rescript는 컴파일 시간에 오류를 보고한다.
 * 다른 언어와 다르게, Rescript는 타입 주석이 정확하다고 가정하지 않는다.
...
/* 타입 인자
 * 타입은 다른 언어의 제네릭과 유사한 인자를 허용한다.
 * 중복을 없애기 위해 고안되었으며, 인자 이름 앞에 ' 를 기입하면 된다.
 * 타입 시스템은 '+인자이름 표기를 통해 (int, int, int)와 같은 타입 선언이 불필요해진다.
 ...
 * 함수와 마찬가지로 rec 키워드를 사용하여 자신을 참조할 수 있다.
 */
 type rec person = {
     name : string,
 ...
 * 그러나 Rescript는 실용성을 고려해 타입 시스템에 거짓말 할 수 있는 단일 타입 탈출구를 제공한다.
