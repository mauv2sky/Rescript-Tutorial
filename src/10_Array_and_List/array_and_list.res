/* Array
 * 배열은 기본적으로 정렬된 자료 구조로 자바스크립트의 배열과 동일하게 작동한다.
 * 리스크립트의 배열의 항목들은 모두 같은 타입이어야 한다.
...
/* List
 * 리스크립트는 단일 연결 리스트 또한 제공한다. 아래의 특징을 가진다.
    - 불변하다. (immutable)
 ...
 */
 ...
 // b의 각 항목이 c의 머리에 하나씩 추가되기 때문에 우연하게 선형 연산(O(b))이 된다.
 // 여러 리스트를 한번에 추가하고 싶다면 List.concat을 사용할 수 있지만 권장되는 방법은 아니다.

 // 리스트의 항목에 접근할 때는 switch를 사용할 수 있다.
 let message = 
    switch myList {
