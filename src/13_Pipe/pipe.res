/* Pipe
 * 리스크립트는 파이프라고 부르는 작지만 놀랍도록 유용한 -> 연산자를 제공한다.
 * -> 연산자를 사용하면 코드의 안과 밖을 뒤집을 수 있다. 즉 a(b)가 b->a로 표현된다.
 * 런타임 비용이 들지 않는 간단한 구문이다.
 * 함수 호출문이 여러 개의 함수로 중첩되어 있을 때 이를 쉽게 읽기가 힘들다.
 * 파이프를 사용하면 같은 함수 호출문이라도 간소화하여 읽기 쉽게 표현할 수 있다.
 */
 // 원본 코드
 /*
...
/* Tips & Tricks
 * 파이프는 정확한 목적을 위해 사용되어야 하며 남용해서는 안된다.
 * 파이프를 활용하기 위해 라이브러리의 API를 만드는 경우가 있는데 이는 잘못된 생각이다.
 */


/* JS Method Chaining
 * 이 개념을 시작하기에 앞서 리스크립트의 binding API에 대한 사전지식이 요구된다.
 * https://rescript-lang.org/docs/manual/latest/bind-to-js-function#object-method
 * 자바스크립트의 API는 종종 객체와 연결되며 다음과 같이 체인이 가능하다.
...
/* Pipe Into Variants
 * 함수처럼 파이프를 배리언트의 생성자로 만들 수 있다.
     let result = name -> preprocess -> Some
     // 다음 코드와 동일하다.
     let result = Some(preprocess(name))
 * 배리언트 생성자를 함수처럼 사용하는 것은 여기 외의 다른 곳에서는 동작하지 않는다.
 */


/* Pipe Placeholders
 * 플레이스홀더는 밑줄로 작성되며 나중에 함수의 인수를 채울 것임을 리스크립트에게 알린다.
 * 다음 두 가지는 동일한 의미를 갖는다.
     let addTo7 = (x) => add3(3, x, 4)
     let addTo7 = add3(3, _, 4)
 * 가끔 내가 가지고 있는 값을 첫번째 위치로 파이프 시키고 싶지 않을 수 있다.
 * 이런 경우에 플레이스홀더 값을 표시하여 파이프할 인수를 표시할 수 있다.
     makePerson(~age=47, ())
        -> namePerson("Jane")
 * person 객체에 적용하고 싶은 name이 있을 경우, 플레이스홀더를 사용할 수 있다.
     getName(input)
        -> namePerson(personDetails, _)
 * 파이프를 통해 원하는 인자의 위치로 값을 전달할 수 있다. 이름이 있는 인자에 대해서도 작동한다.
     getName(input)
