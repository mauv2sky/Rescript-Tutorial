 ...
 * 리스크립트는 null과 undefined 개념이 존재하지 않는다.
 ...
 * 하지만, 잠재적으로 존재하지 않는 값에 대한 개념은 유용하며 이 값들은 리스크립트 내에서 안전한 상태로 존재하므로 알아둘 필요가 있다.
 */
 ...
 // 나중에 다른 코드가 위의 값을 받으면 패턴 매칭을 통해 두 경우를 모두 처리해야 한다.
 ...
 // 순수한 리스크립트 프로그램에는 null 오류가 발생하지 않는다.
 ...
 */
 ...
 // 주의할 점 1
 // 리스크립트에서는 option을 구성할 수 있기 때문에, option에서 undefined로 변환되는 것은 완벽하지 않다.
 ...
 // 리스크립트의 option 타입은 None을 다룰 때 undefined 또는 not null만을 확인하기 때문이다.
 ...
 // 반대로, 리스크립트에서 nullable 문자열을 생성할 경우에는 다음과 같이 작성한다.
